...
--Display unique categories in the retail_sales table
SELECT DISTINCT category FROM retail_sales;
-- Null value check:Check for any null values in the dataset and delete records with missing data.
SELECT * FROM retail_sales
WHERE
sale_date IS NULL OR sale_time IS NULL OR customer_id IS NULL OR gender IS NULL OR age IS NULL OR category IS NULL
...
--02. Write a SQL query to retrieve all transactions where the category is 'Clothing' and the quantity sold is 
...
AND quantity >= 3;
--03. write a SQL query to calculate the total sales (total_sales) for each category.
SELECT category,
       sum(total_sale) as net_sale,
	   COUNT(*) as total_orders
FROM retail_sales
GROUP BY 1;
--04. Write a SQL query to find out the average age of customers who purchased items from the 'Beauty' category.
...
FROM retail_sales
GROUP BY category;
--10. Write a sql uery to create each shift and number of orders (Example Morning<12,Afternoon Between 12 & 17, Evening >17):)
WITH hourly_sale
AS
(
SELECT *,
